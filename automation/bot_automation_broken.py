...
TRIGGER_MESSAGE = os.getenv("TRIGGER_MESSAGE", "Появились новые перевозки")
...
class TransportBookingBot:
    ...
    def __init__(self):
        ...
        self.last_processed_message_id = None  # ID последнего обработанного сообщения для защиты от дублей
    ...
    async def continue_automation(self, message: Message):
        ...
        # Защита от дублированной обработки одного и того же сообщения
        if self.last_processed_message_id == message.id:
            logger.debug(f"⏭️  Пропускаем: сообщение {message.id} уже обработано")
            return
        ...
        if self.automation_start_time:
            ...
            if elapsed > 10.0:  # Таймаут 10 секунд (увеличен)
    ...
    async def process_new_transport(self, message: Message):
        ...
        try:
            ...
            if self.last_keyboard:
                ...
                self.last_processed_message_id = None  # Сброс для новой автоматизации
